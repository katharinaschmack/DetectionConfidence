...
if BpodSystem.Data.Custom.LaserTrial(iTrial)==1
    sma = AddState(sma, 'Name', 'StartRecording',...
        ...
        'OutputActions', {'BNCState', 1}); % trigger photometry acq global timer, nidaq trigger, point grey camera
...
if BpodSystem.Data.Custom.PhotometryOn(iTrial)==1||BpodSystem.Data.Custom.PhotometryOn(iTrial)==2
    ...
    'Timer', 0.01,...
    'StateChangeConditions', {CenterPortIn, 'Cin_PhotometryStart'},...
    'OutputActions', [NoiseStartOutput LED.wait_Cin]);%HERe

    sma = AddState(sma, 'Name', 'Cin_PhotometryStart',...
        'Timer', 0.025,...
        'StateChangeConditions', {'Tup', 'Cin_GraceStart'},...
        'OutputActions', {'BNCState', npgBNCArg});
else
    sma = AddState(sma, 'Name', 'wait_Cin',...
    'Timer', 0.01,...
    'StateChangeConditions', {CenterPortIn, 'Cin_GraceStart'},...
    'OutputActions', [NoiseStartOutput LED.wait_Cin]);%HERe
%record trigger photometry at poke in
end %% get baseline photometry without behavior
...
sma = AddState(sma, 'Name', 'wait_Lin',...
    ...
    'StateChangeConditions', {CorrectPortIn, 'LinCorrect_GraceStart',ErrorPortIn, 'LinError_GraceStart'},...
...
sma = AddState(sma, 'Name', 'LoutError_GracePeriod',...
    'Timer',TaskParameters.GUI.FeedbackDelayGrace, ...
    'StateChangeConditions', {ErrorPortIn,'LinError_PreFb','Tup','Error_Timeout'},...
